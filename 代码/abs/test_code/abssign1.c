...
abssign1(  char* sk, char* ac)
{
	...
	for(i = 3; i >= 0; i-- )
		fputc('a'+((file_len & 0xff<<(i*8))>>(i*8)), stdout);
	...
	siglen = sig_buf->len * 2;
	for( i = 3; i >= 0; i-- )
		fputc('a'+((siglen & 0xff<<(i*8))>>(i*8)), stdout);
	for(i = 0; i < sig_buf->len; i++)
	{
		fputc('a'+(sig_buf->data[i]&0xf), stdout);
		fputc('a'+((sig_buf->data[i]>>4)&0xf), stdout);
	}
	
	unsigned char msglen;
	msglen = message_buf->len * 2;
	for( i = 3; i >= 0; i-- )
		fputc('a'+((msglen & 0xff<<(i*8))>>(i*8)), stdout);
	for(i = 0; i < message->len; i++)
	{
		fputc('a'+(message->data[i]&0xf), stdout);
		fputc('a'+((message->data[i]>>4)&0xf), stdout);
	}
