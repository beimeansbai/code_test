...
int
absverify_test( char* sign_file, char* cipher_file, char* ac, int k)
{
	mprabs_pub_t* pub;
	mprabs_sig_t* sig;
	int file_len;
    ...
    read_abs_file(in_file, &cph_buf, &file_len, &sig_buf);
    //int i;
	//int len;
	//char ch;
	...
	//aaaaa
	/*FILE * middle;
	if( !(middle = fopen("/home/xx/test_file/middle.txt", "w")) )
		printf("can't write file: '/home/xx/test_file/middle.txt'\n");
	...
	//aaaaaaa
	//read_abs_file("/home/xx/test_file/middle.txt", &cph_buf, &file_len, &sig_buf);
...
void main()
{
	int a = absverify_test( "signedtext.txt", "ciphertext1.txt", "security_team and (abc or (hire_data and business_staff))", 1);
